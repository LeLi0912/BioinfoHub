...
const CompareTable: React.FC<Props> = ({ items, onRemove }) => {
  if (items.length === 0) {
    return <div style={{ color: '#999', textAlign: 'center', padding: 40 }}>暂无对比工具，请从详情页添加或搜索添加</div>;
  }
  ...
  const data = [
    ...
    { label: '协议', ...Object.fromEntries(items.map((t) => [t.slug, t.github_license || '-'])) },
    {
      label: '活跃度',
      ...Object.fromEntries(
        items.map((t) => [t.slug, t.is_active ? <Tag color="green">活跃</Tag> : <Tag color="red">停更</Tag>])
      ),
    },
    {
      label: '创建时间',
      ...Object.fromEntries(items.map((t) => [
        t.slug,
    ...
    },
    {
      ...
      ...Object.fromEntries(items.map((t) => [
        t.slug,
        <Space wrap>{t.categories.map((c) => <Tag key={c.id} color="blue">{c.name}</Tag>)}</Space>,
      ])),
    },
  ];
...
function _highlight(value: number, max: number): React.ReactNode {
  if (value === max && max > 0) {
    return <span style={{ color: '#52c41a', fontWeight: 'bold' }}>{value.toLocaleString()} ★</span>;
  }
