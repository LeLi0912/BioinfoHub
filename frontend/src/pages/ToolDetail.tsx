...
const ToolDetail: React.FC = () => {
  ...
  return (
    <div>
      ...
      <Typography.Paragraph style={{ color: '#6b6b6b', fontSize: 15, marginBottom: 20 }}>
        {desc}
      </Typography.Paragraph>

      <Space style={{ marginBottom: 24 }}>
        {tool.github_url && (
          <Button icon={<GithubOutlined />} href={tool.github_url} target="_blank">
            GitHub
          </Button>
        )}
        {tool.homepage_url && (
          <Button icon={<HomeOutlined />} href={tool.homepage_url} target="_blank">
            官网
          </Button>
        )}
        {tool.publication_doi && (
          <Button
            ...
            href={`https://doi.org/${tool.publication_doi}`}
            target="_blank"
          >
            论文
          </Button>
      ...
      <Descriptions bordered column={{ xs: 1, sm: 2, md: 3 }} size="middle">
        ...
        <Descriptions.Item label="编程语言">
          {tool.github_language ? (
            <Tag style={{ borderRadius: 3, border: '1px solid #e8e4df', background: '#faf9f6' }}>
              {tool.github_language}
            </Tag>
          ) : '-'}
        </Descriptions.Item>
        ...
        <Descriptions.Item label="GitHub 创建">
          {tool.github_created_at
            ? new Date(tool.github_created_at).toLocaleDateString('zh-CN')
            : '-'}
        </Descriptions.Item>
        <Descriptions.Item label="最近更新">
          {tool.github_updated_at
        ...
        {tool.publication_title && (
          <Descriptions.Item label="相关论文" span={3}>
            <span style={{ fontFamily: "'Noto Serif SC', serif", fontStyle: 'italic' }}>
              {tool.publication_title}
            </span>
            {tool.publication_year && ` (${tool.publication_year})`}
            {tool.citation_count > 0 && (
              <span style={{ color: '#2d6a4f', marginLeft: 8 }}>— 引用 {tool.citation_count}</span>
            )}
          </Descriptions.Item>
        )}
        {tool.github_topics && tool.github_topics.length > 0 && (
          <Descriptions.Item label="Topics" span={3}>
            <Space wrap>
              {(tool.github_topics as string[]).map((t: string) => (
                <Tag
                  key={t}
                  style={{ borderRadius: 3, border: '1px solid #e8e4df', background: '#ffffff', color: '#6b6b6b' }}
                >
                  {t}
                </Tag>
              ))}
